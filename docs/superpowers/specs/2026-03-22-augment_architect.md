下面是 **`skills/repo-structure/prompts/augment_architect.md` 初稿**。
口径对齐你前面已经定下来的边界：`augment` 不是全文档总结，也不是让模型自己逛 repo，而是 **Python 先收 candidate evidence，LLM 再做 claim adjudication**。这和你文档里对 `augment` 的两段式设计、状态枚举，以及 `architect_augment.yaml` 以 evidence-backed claim 为核心的定义是一致的。
...
这一版和 `extract_codebase.md` 是配套的：  
`extract` 负责从 dossier section 里抽原子 fact；`augment` 负责把 architect claims 判成 `evidence_backed / weakly_backed / gap / drift`，而且只基于外部收集好的 candidates 做裁决，不自己扩边界。
